...
class Hoist_crane_model extends App_Model
{
    ...
    public function get($id = '',  $where = [])
    {
        if (is_numeric($id)) {
            ...
            $category = $this->db->get(db_prefix() . 'hoist_crane')->row();

            return $category;
        }
        $this->db->select('*,' . db_prefix() . 'hoist_crane.id');
        $this->db->from(db_prefix() . 'hoist_crane');
        $this->db->where($where);
    ...
    public function create($data){
        ...
        $this->db->insert(db_prefix().'hoist_crane', $data);
        $equipment_id = $this->db->insert_id();

        hooks()->do_action('after_hoist_crane_added', $equipment_id);
        return $equipment_id;
    }
    ...
    public function update($data, $rel_id, $task_id){
        ...
        $this->db->where('rel_id', $rel_id);
        $this->db->where('task_id', $task_id);
        $this->db->update(db_prefix() . 'hoist_crane', $data);
    }
