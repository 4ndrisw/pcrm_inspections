...
class Scissor_lift_model extends App_Model
{
    ...
    /**
     * Get scissor_lift/s
     * @param mixed $id scissor_lift id
    ...
    public function get($id = '',  $where = [])
    {
        if (is_numeric($id)) {
            ...
            $category = $this->db->get(db_prefix() . 'scissor_lift')->row();

            return $category;
        }
        $this->db->select('*,' . db_prefix() . 'scissor_lift.id');
        $this->db->from(db_prefix() . 'scissor_lift');
        $this->db->where($where);
    ...
    public function create($data){
        ...
        $this->db->insert(db_prefix().'scissor_lift', $data);
        $equipment_id = $this->db->insert_id();

        hooks()->do_action('after_scissor_lift_added', $equipment_id);
        return $equipment_id;
    }
    ...
    public function update($data, $rel_id, $task_id){
        ...
        $this->db->where('rel_id', $rel_id);
        $this->db->where('task_id', $task_id);
        $this->db->update(db_prefix() . 'scissor_lift', $data);
    }
