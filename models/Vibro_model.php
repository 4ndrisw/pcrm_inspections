...
class Vibro_model extends App_Model
{
    ...
    public function get($id = '',  $where = [])
    {
        if (is_numeric($id)) {
            ...
            $category = $this->db->get(db_prefix() . 'vibro')->row();

            return $category;
        }
        $this->db->select('*,' . db_prefix() . 'vibro.id');
        $this->db->from(db_prefix() . 'vibro');
        $this->db->where($where);
    ...
    public function create($data){
        ...
        $this->db->insert(db_prefix().'vibro', $data);
        $equipment_id = $this->db->insert_id();

        hooks()->do_action('after_vibro_added', $equipment_id);
        return $equipment_id;
    }
    ...
    public function update($data, $rel_id, $task_id){
        ...
        $this->db->where('rel_id', $rel_id);
        $this->db->where('task_id', $task_id);
        $this->db->update(db_prefix() . 'vibro', $data);
    }
