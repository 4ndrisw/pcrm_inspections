...
class Boiler_model extends App_Model
{
    ...
    public function get($id = '',  $where = [])
    {
        if (is_numeric($id)) {
            ...
            $category = $this->db->get(db_prefix() . 'boiler')->row();

            return $category;
        }
        $this->db->select('*,' . db_prefix() . 'boiler.id');
        $this->db->from(db_prefix() . 'boiler');
        $this->db->where($where);
    ...
    public function create($data){
        ...
        $this->db->insert(db_prefix().'boiler', $data);
        $equipment_id = $this->db->insert_id();

        hooks()->do_action('after_boiler_added', $equipment_id);
        return $equipment_id;
    }
    ...
    public function update($data, $rel_id){
        $this->db->select('id');
        $this->db->where('rel_id', $rel_id);
        $this->db->update(db_prefix() . 'boiler', $data);
    }
